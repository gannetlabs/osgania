...
**Change**: vps-provisioning-base
**Project**: osgania
**Artifact store**: openspec
**Established**: 2026-06-14
**Status**: spec
**Depends on**: proposal.md (APPROVED), design.md (RESOLVED — source of all concrete values)
**Cross-reference**: openspec/specs/platform-security-core/spec.md (the security contract this slice makes enforceable)

This document is the WHAT. Every decided value is copied verbatim from design.md (drift gate: if a value here differs from design.md, design.md wins — fix the spec, not the design). Implementation tasks come from `sdd-tasks`; this spec does not describe HOW.
