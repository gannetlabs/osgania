...
Defines what MUST be true after this change is applied. Implementation decisions (HOW) belong to the design phase. This spec is the contract that bats tests encode — every behavioral scenario here maps to one or more test cases.
...
## Assumptions made at spec time

The following questions were open in the proposal. This spec resolves them with the minimum assumption required to make scenarios concrete and testable.
...
| A6 | guardia match for `platform/` writes | Substring match on `platform/` in the command — design to refine the write-context heuristic | Medium |
...
- `chattr +a` enforcement (provisioning concern)
