...
The probe tests the managed-settings layer (`disableBypassPermissionsMode:"disable"` neutralizing `--dangerously-skip-permissions`), which is entirely independent of the wrapper. The probe MUST export `ANTHROPIC_API_KEY` itself from `CREDENTIALS_DIRECTORY` (same `tr -d '[:space:]'` strip pattern as the wrapper) and then invoke `/usr/bin/claude` directly. The probe MUST NOT include `--permission-mode dontAsk`. Cross-ref HB-05.2 and HB-05.4.
